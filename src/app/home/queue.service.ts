...
export const routes = {
  queue: (c: Queue) => `https://sqs.eu-central-1.amazonaws.com/800070360327/${c.queue}` + (c.attributes?'?':'') + (c.attributes?c.attributes:[]).map((it:any) => `&${Object.keys(it)[0]}=${it[Object.keys(it)[0]]}`)
};
